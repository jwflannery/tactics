...
namespace CreativeSpore.SuperTilemapEditor
{
    public class RogueLikeMapGenerator : MonoBehaviour 
    {
        ...
        private void Update()
        {
            currentTile = GroundOverlay.GetTile(TilemapUtils.GetMouseGridX(GroundOverlay, Camera.main), TilemapUtils.GetMouseGridY(GroundOverlay, Camera.main));
            if (currentTile != null && currentTile.paramContainer.FindParam("name") != null)
            {
                myText.text = currentTile.paramContainer.FindParam("name").ToString();
            }
        }
